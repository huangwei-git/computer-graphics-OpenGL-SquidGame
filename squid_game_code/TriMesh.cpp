...
void TriMesh::setHeight(float h) {
	height = h;
}

float TriMesh::getHeight() {
	return height;
}

void TriMesh::setWidth(float w) {
	width = w;
}

float TriMesh::getWidth() {
	return width;
}

void TriMesh::setLength(float l) {
	length = l;
}

float TriMesh::getLength() {
	return length;
}
...
void TriMesh::storeFacesPoints()
{
	...
	if (do_normalize_size) {
		...
		diagonal_length = glm::length( up_corner - down_corner );
	...
	for (int i = 0; i < faces.size(); i++)
	{
		...
		if (vertex_normals.size() > 0) {
			...
			// 颜色
			colors.push_back(vertex_normals[normal_index[i].x]);
			colors.push_back(vertex_normals[normal_index[i].y]);
			colors.push_back(vertex_normals[normal_index[i].z]);
		}
		// 纹理
...
void TriMesh::generateSquare(glm::vec3 color,float width,float height)
{
	// 创建顶点前要先把那些vector清空
	cleanData();
	glm::vec3 s_vertices[4] = {
...
void TriMesh::readObj(const std::string& filename)
{
	...
	while (std::getline(fin, line))
	{
		...
		std::string type;
		GLfloat _x, _y, _z;
		int a0, b0, c0;//面片第0个顶点的位置/纹理/法线索引
		int a1, b1, c1;//面片第1个顶点的位置/纹理/法线索引
		int a2, b2, c2;//面片第2个顶点的位置/纹理/法线索引
		char slash;
...
void TriMesh::readMtl(const std::string& filename) {
	...
	while (std::getline(fin, line))
	{
		...
		if (type == "Ka") {
			sin >> r0 >> g0 >> b0;
			setAmbient(glm::vec4(r0, g0, b0, 1.0));
		}
		//读取对漫反射的反射系数
		else if (type == "Kd") {
			sin >> r1 >> g1 >> b1;
			setDiffuse(glm::vec4(r1, g1, b1, 1.0));
		}
		//读取对镜面反射的反射系数
		else if (type == "Ks") {
			sin >> r2 >> g2 >> b2;
			setSpecular(glm::vec4(r2, g2, b2, 1.0));
		}
		//读取高光系数系数
		else if (type == "Ns") {
			sin >> s;
	...
	if (s <= 0) setShininess(1.0);
}



// Light
glm::mat4 Light::getShadowProjectionMatrix(int type, float offset) {
	...
	return glm::mat4(
		-(ly- offset), 0.0, 0.0, 0.0,
		lx, offset, lz, 1.0,
		0.0, 0.0, -(ly- offset), 0.0,
		-lx* offset, -ly* offset, -lz* offset, -ly);
